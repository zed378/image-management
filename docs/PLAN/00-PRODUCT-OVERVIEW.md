...
Specify product overview for the Image Management & Delivery Platform. Defines what the platform is, who it is for, and what it must do before any code is written.
...
- Define the concrete rules for product overview -- concepts alone are not sufficient; every rule must be specific enough to write a test against.
- State explicit defaults for every configurable value related to product overview.
