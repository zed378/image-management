...
describe("redactUrl", () => {
  it.each([
    ...
    ["https://cdn.example.com/i/abc?signature=x&h=10", `https://cdn.example.com/i/abc?signature=${REDACTED}&h=10`],
    [
  ...
  it("keeps transformation parameters intact, since they are what an operator debugs", () => {
    expect(redactUrl("/i/abc?w=800&h=600&fit=cover&f=auto")).toBe("/i/abc?w=800&h=600&fit=cover&f=auto");
  });
