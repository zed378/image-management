...
Set numeric latency/throughput targets specific to the delivery path, feeding `docs/PERFORMANCE/03-IMAGE-DELIVERY-PERFORMANCE.md`.
...
- Target warm-cache (CDN hit) latency and cold-cache (full pipeline) latency separately -- they are different budgets with different owners (CDN provider vs. Image Processing Engine).
- State these as the numbers `docs/PERFORMANCE/00-PERFORMANCE-REQUIREMENTS.md` and `TASKS/PHASE-7...` P7-05's load tests are graded against -- this document is upstream of that testing task, not a duplicate of it.
