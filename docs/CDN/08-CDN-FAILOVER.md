...
Specify cdn failover for the Image Management & Delivery Platform. Define how derivatives reach the edge: origin shield, cache key composition, TTL policy, and purge/invalidation API.
...
- Define the behavior when the primary CDN or origin is unreachable: secondary CDN, direct-to-storage fallback, or serving a stale-but-cached derivative past its TTL (stale-while-revalidate).
