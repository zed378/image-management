...
The single authoritative table of every parameter this protocol accepts: name, aliases, type, accepted range/values, default, and which other parameters it interacts with.
...
- One row per parameter (`w`/`width`, `h`/`height`, `fit`, `position`, `focus`, `ratio`, `q`/`quality`, `format`, `dpr`, `bg`, `rotate`, `blur`, `sharpen` if offered) -- this table is the source every other protocol document and `docs/API/13-IMAGE-TRANSFORMATION-API.md` must match exactly, never re-specify independently.
- Mark each parameter's interaction with others explicitly (e.g. `fit=contain` + `bg=` are coupled; `focus=` only applies when `fit=cover`).
- Version every addition/removal of a parameter against `30-VERSIONING.md` -- a new parameter is a minor, additive change; a changed default or removed parameter is a breaking one.
