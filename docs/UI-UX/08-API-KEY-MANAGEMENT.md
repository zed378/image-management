# 08 - API Key Management UI

> Category: **Dashboard UI/UX** (`docs/UI-UX/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Specify api key management ui for the Image Management & Delivery Platform. Define the full key lifecycle: creation (scoped, shown once), listing (hash only, never plaintext, after creation), rotation (overlap window so old key keeps working briefly), and revocation (immediate).
