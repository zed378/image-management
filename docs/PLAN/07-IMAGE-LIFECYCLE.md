...
Specify image lifecycle for the Image Management & Delivery Platform. Model every state an asset/image can be in (uploading, validating, processing, ready, failed, soft-deleted, purged) as an explicit state machine with allowed transitions and the event/webhook fired on each transition.
