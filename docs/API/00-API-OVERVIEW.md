...
Specify api overview for the Image Management & Delivery Platform. The platform is API-first.
...
- Define the concrete rules for api overview -- concepts alone are not sufficient; every rule must be specific enough to write a test against.
- State explicit defaults for every configurable value related to api overview.
