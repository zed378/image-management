...
Enumerate every `fit` mode precisely, since this is the parameter most likely to produce a visually wrong-but-technically-successful result if under-specified.
...
- `cover`: scale to fill the box, cropping overflow -- the default when both `w` and `h` are given.
- `contain`: scale to fit within the box, letterboxing with `bg=` (default transparent where the format supports it, else white).
- `fill`: stretch to the exact box, ignoring aspect ratio -- must be opt-in only, never a default, since it distorts the image.
- `inside`/`scale-down`: like `contain` but never upscale past the original's dimensions -- important for consumers that don't want to pay the quality cost of upscaling.
