# 26 - Content-Type Rules

> Category: **Image Delivery Protocol** (`docs/IMAGE-DELIVERY-PROTOCOL/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Define the exact `Content-Type` returned per output format, and the rule for when the format is negotiated (`format=auto`).
...
- One-to-one mapping: `image/jpeg`, `image/png`, `image/webp`, `image/avif`, `image/gif` -- never a generic `application/octet-stream` for a successful response.
- Confirm the returned `Content-Type` always matches the *actual* bytes returned (post-negotiation), never the originally-requested-but-overridden one.
