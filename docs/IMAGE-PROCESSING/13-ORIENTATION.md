...
Specify orientation for the Image Management & Delivery Platform. Always read and apply EXIF orientation before any other transformation, then strip the orientation tag from the output so downstream consumers never double-rotate.
...
- Always read and apply EXIF orientation before any other transformation, then strip the orientation tag from the output so downstream consumers never double-rotate.
