...
## Purpose
 ...
 Define support for HTTP `Range` requests (partial content), relevant for large originals or progressive-loading consumer UIs.
...
- Support `Range` on `GET` for both originals and derivatives where the storage/CDN layer supports it; return `206 Partial Content` with `Content-Range`, or `416` for an unsatisfiable range.
- State whether range support is required for v1 or explicitly deferred (many image-delivery use cases don't need it since images are typically small enough to fetch whole) -- don't leave it silently unspecified either way.
