...
Define delivery-protocol-specific abuse patterns and mitigations, complementing `docs/SECURITY/16-ABUSE-PREVENTION.md`.
...
- Parameter-fuzzing abuse: an attacker requesting thousands of near-identical dimensions (`w=800`, `w=801`, `w=802`, ...) to force cache-miss-triggering, compute-expensive regeneration on every request -- mitigate via a discrete allowed-dimension set or a stricter cache-miss-specific rate limit (per `33-RATE-LIMITS.md`).
- Hotlinking/bandwidth theft: optional referrer allow-listing per application, enforced at the edge where possible to avoid even reaching origin.
