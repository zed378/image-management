...
beforeAll(async () => {
  ...
  app.get("/t/zod", async (request) => {
    z.object({ w: z.coerce.number().int().max(8192) }).strict().parse(request.query);
    return {};
  });
  app.post(
    "/t/schema",
    { schema: { body: { type: "object", required: ["name"], additionalProperties: false, properties: { name: { type: "string" } } } } },
    async () => ({}),
  );
});

const errorOf = (res: { json: () => unknown }) => (res.json() as { error: Record<string, unknown> }).error;
...
describe("error envelope mapping", () => {
  ...
  it("replaces a 5xx AppError's message with the generic one and adds Retry-After", async () => {
    ...
    expect(errorOf(res)).toMatchObject({ code: "storage_unavailable", message: "Storage is temporarily unavailable." });
  ...
  it("turns an unknown error into a generic 500 and logs the real cause", async () => {
    ...
    const failure = logs().find((l) => l["msg"] === "request failed" && l["request_id"] === res.headers["x-request-id"]);
  ...
  it("maps a route schema failure to 400 validation_failed", async () => {
    ...
    expect(errorOf(res)["details"]).toEqual(expect.arrayContaining([{ field: "name", reason: "required" }]));
  });
  ...
  it("logs a 4xx at warn and a 5xx at error", async () => {
    ...
    const lineFor = (id: unknown, msg: string) => logs().find((l) => l["request_id"] === id && l["msg"] === msg);
