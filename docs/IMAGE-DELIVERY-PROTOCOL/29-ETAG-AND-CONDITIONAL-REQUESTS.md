...
Define `ETag` generation and support for `If-None-Match`/`If-Modified-Since` conditional requests.
...
- `ETag` derives from the same derivative identity as `17-DERIVATIVE-IDENTITY.md` (e.g. the `derivative_id` itself, possibly weak-tagged) so it's cheap to compute without re-reading the full object.
- A matching `If-None-Match` returns `304 Not Modified` with no body -- still re-runs authorization for PRIVATE/SIGNED assets exactly as a full `GET` would, per `25-IMAGE-HEAD-REQUEST.md`'s same principle.
