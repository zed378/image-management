...
Define the `expires=` mechanism precisely: format, clock-skew tolerance, and behavior once expired.
...
- `expires=` is a Unix timestamp (seconds); reject with a specific, distinct error code (not the generic signature-invalid one) when expired, so client SDKs and consumers can tell "too late" apart from "tampered".
- Define an allowed clock-skew tolerance (e.g. 30-60 seconds) to avoid spurious failures from minor client/server clock drift, without meaningfully weakening the expiry guarantee.
