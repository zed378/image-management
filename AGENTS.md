...
| Path | What it holds |
|---|---|
| `docs/` | **The specification.** 293 documents across 20 categories: product, architecture, API contract, database schema, the image delivery protocol, image processing, storage, multi-tenancy, security, CDN, search, SDKs, webhooks, observability, performance, DevOps, testing, dashboard UI, developer docs. Every document starts life as "Draft specification" and is flipped to "Final" by the task that implements it. |
| `TASKS/` | **The execution plan.** 74 tasks across 8 phases (`P0`-`P7`), each naming the `docs/` files it implements and its Definition of Done. Start at `TASKS/PROGRESS.md`. Conventions in `TASKS/00-TASK-CONVENTIONS.md` are mandatory, not optional reading. |
...
2. **Read every document that task's `Implements:` list names, in full.**
...
3. **Read the `MEMORY/records/` file for every task this one depends on.**
   ...
   sessions.
4. **Do the work** against the task's own Steps and Definition of Done,
   plus the baseline Definition of Done in `TASKS/00-TASK-CONVENTIONS.md`.
5. **Update the `docs/` files** the task implements from "Draft
   specification" to "Final" (or note explicitly what's still open).
6. **Write `MEMORY/records/{TASK-ID}.md`** using
   `MEMORY/records/TEMPLATE.md`. Add a `MEMORY/DECISIONS.md` entry if a
   real, non-obvious fork in the road was resolved.
7. **Update `TASKS/PROGRESS.md`** in the same change.
8. If truly blocked (a decision is missing that this task cannot make
