# 20 - CDN Delivery Protocol

> Category: **Image Delivery Protocol** (`docs/IMAGE-DELIVERY-PROTOCOL/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Define the contract between this protocol and the CDN/edge layer: what the edge is allowed to cache, how it forwards cache-miss traffic to origin, and what headers it must pass through unmodified.
...
- The edge treats the full canonical URL (or the derived cache key, if the CDN supports custom cache-key rules) as opaque -- it must never itself attempt to interpret or normalize transformation parameters; that's origin's (this protocol's) job exclusively, to avoid two divergent normalization implementations.
- Require the edge to forward `Accept` (for format negotiation, with `Vary: Accept` honored) and `Authorization`/signature query params through to origin on cache miss, unmodified.
