...
Define the protocol's own version scheme, independent of (but coordinated with) `docs/API/04-API-VERSIONING.md`.
...
- Version lives in the URL path (`/v1/assets/...`) from day one -- never add it retroactively once consumers exist.
- Document, per version, its supported parameter set, deprecated parameters (still accepted, logged, scheduled for removal), and behavior guarantees that will not change within that version's lifetime.
...
v1
 ...
 +-- behavior guarantees (frozen for this version's lifetime)

v2
 +-- future evolution (additive where possible; breaking changes only
     under a new version segment)
```
