...
Define the backward-compatibility guarantee within a protocol version and the deprecation process across versions.
...
- Within `v1`: adding a new parameter is always compatible; changing an existing parameter's default or removing a parameter is a breaking change and requires a new version segment, no exceptions.
- Define the deprecation window (announce -> dual-support period -> removal) and where deprecation notices surface to consumers (response header, e.g. `X-Deprecated-Parameter`, plus `docs/DEVELOPER/13-MIGRATION.md`).
