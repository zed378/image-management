...
Specify authorization for the Image Management & Delivery Platform. Covers authentication, authorization, the platform's threat model, and the controls that defend against IDOR/BOLA, malicious uploads, credential abuse, and denial of service.
...
- Define the concrete rules for authorization -- concepts alone are not sufficient; every rule must be specific enough to write a test against.
- State explicit defaults for every configurable value related to authorization.
