...
Specify audit logging for the Image Management & Delivery Platform. An append-only log of security-relevant actions (key created/revoked, asset deleted, visibility changed, admin override) with actor, action, target, and timestamp -- never mutated, only inserted.
