# 36 - Delivery Observability

> Category: **Image Delivery Protocol** (`docs/IMAGE-DELIVERY-PROTOCOL/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Define the delivery-path-specific metrics this protocol needs beyond the general set in `docs/OBSERVABILITY/`.
...
- Cache hit ratio broken down by cache-miss *reason* (cold/never-requested vs. evicted vs. invalidated-by-new-version) -- a single aggregate hit-ratio number hides which failure mode to fix.
- Format-negotiation outcome distribution (AVIF vs. WebP vs. JPEG served) as a direct measure of the bandwidth-optimization value described in `12-FORMAT-NEGOTIATION.md`.
