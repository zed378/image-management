# 13 - DPR & Responsive Images

> Category: **Image Delivery Protocol** (`docs/IMAGE-DELIVERY-PROTOCOL/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Define the `dpr=` parameter for high-density (Retina) displays and how it composes with `w=`/`h=`.
...
- `dpr=2` multiplies the *requested* pixel dimensions by 2 before rendering (i.e. `w=400&dpr=2` renders at 800px physical width) while the parameter itself stays part of the cache key/derivative identity like any other.
- Define the accepted DPR range (typically 1-3) and that fractional values (e.g. `dpr=1.5`) are supported since real device pixel ratios are often fractional.
