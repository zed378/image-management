...
Specify image query api for the Image Management & Delivery Platform. Define the declarative query surface consumers use to request a derivative: a REST query-string form (?w=&h=&fit=&...) and, if offered, a path-segment form. The two must be provably equivalent.
