...
Specify idempotency for the Image Management & Delivery Platform. Every non-idempotent write endpoint (upload, bulk delete) must accept an Idempotency-Key header and return the original response on retry rather than creating a duplicate resource.
...
- Every non-idempotent write endpoint (upload, bulk delete) must accept an Idempotency-Key header and return the original response on retry rather than creating a duplicate resource.
