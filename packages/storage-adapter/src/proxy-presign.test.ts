...
const valid = { k: "t/p/originals/a/1.jpg", m: "PUT" as const, e: nowSeconds + 60, ct: "image/jpeg", max: 1024 };
...
describe("verifyProxyToken", () => {
  ...
  it("rejects a token whose payload was edited to point at another key", () => {
    ...
    const forged = Buffer.from(JSON.stringify({ ...valid, k: "other-tenant/p/originals/a/1.jpg" })).toString("base64url");

    expect(() => verifyProxyToken(`${forged}.${signature}`, "PUT", SECRET, NOW)).toThrow(/signature/);
  });
  ...
  it("rejects a token used for another method", () => {
    expect(() => verifyProxyToken(createProxyToken(valid, SECRET), "GET", SECRET, NOW)).toThrow(/different method/);
  });
...
describe("withProxyPresign", () => {
  ...
  it("issues a proxy PUT URL on the platform's own host", async () => {
    const put = await presigning.presignPut("t/p/a.jpg", { expiresInSeconds: 300, contentType: "image/jpeg" });
  ...
  it("validates the key before signing anything", async () => {
    await expect(presigning.presignGet("../x", { expiresInSeconds: 60 })).rejects.toBeInstanceOf(StorageKeyError);
  });

  it("refuses a secret shorter than 32 bytes", () => {
    expect(() => withProxyPresign(new MemoryStorageAdapter(), { baseUrl: "https://x", secret: "short" })).toThrow(
      /at least 32 bytes/,
    );
  });
});
