...
Specify api key management for the Image Management & Delivery Platform. Define the full key lifecycle: creation (scoped, shown once), listing (hash only, never plaintext, after creation), rotation (overlap window so old key keeps working briefly), and revocation (immediate).
