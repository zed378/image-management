# 25 - Image HEAD Request

> Category: **Image Delivery Protocol** (`docs/IMAGE-DELIVERY-PROTOCOL/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Define `HEAD` request support: returning the same headers (Content-Type, Content-Length, ETag, Cache-Control) a `GET` would, without transferring the body.
...
- A `HEAD` request must still run parameter validation and authorization exactly as a `GET` would (so a `HEAD` cannot be used to probe for a resource's existence or bypass a signature check) -- only the body transfer is skipped, not any check.
- Note this lets a consumer application check a derivative's `Content-Length`/dimensions before deciding to fetch it, without incurring bandwidth cost.
