...
Define TTL and cache-invalidation policy specific to the protocol layer (distinct from, and consumed by, `docs/CDN/02-CACHE-CONTROL.md` and `04-CACHE-TTL.md`).
...
- A derivative's TTL is a function of the owning asset's visibility class (`docs/PLAN/21-ASSET-VISIBILITY.md`): long/immutable for PUBLIC content-addressed derivatives, capped at the signature's own expiry for SIGNED/EXPIRING.
- State the invalidation trigger set precisely: new asset version (invalidates all derivatives of the prior version), explicit purge API call, visibility change (PUBLIC -> PRIVATE must stop serving from any cache layer immediately, not just at next TTL expiry).
