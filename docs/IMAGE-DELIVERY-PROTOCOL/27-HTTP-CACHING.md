...
Define standard HTTP caching semantics (distinct from CDN-internal caching in `19-CACHE-CONTROL.md`/`docs/CDN/`) for browser- and intermediate-proxy-level caching.
...
- Set `Cache-Control`, `Expires` (legacy fallback), and `Last-Modified` consistently with `19-CACHE-CONTROL.md`'s TTL policy.
- Content-addressed derivatives (immutable once created) should use `Cache-Control: public, max-age=<long>, immutable` so browsers never even issue a conditional revalidation request.
