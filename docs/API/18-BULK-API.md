...
Specify bulk api for the Image Management & Delivery Platform. Define batch semantics explicitly: is it all-or-nothing or best-effort per item, what the response shape looks like for a partial failure, and the max batch size.
...
- Define batch semantics explicitly: is it all-or-nothing or best-effort per item, what the response shape looks like for a partial failure, and the max batch size.
