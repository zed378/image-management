...
### ADR-020: Fastify 5 as the HTTP framework
- Status: Accepted
- Date: 2026-09-21
- Context: `P0-08` had to choose the framework the api is built on. The
  requirements that matter here: the delivery path must be cheap per request
  (it sits behind a CDN but serves every miss), request lifecycle hooks must
  be explicit enough to put authentication, tenant context, rate limiting,
  and permission checks in a fixed order (`docs/ARCHITECTURE/04`), the logger
  must be our pino instance with our redaction list, and the app must be
  testable without binding a port.
- Decision: **Fastify 5.** `buildApp()` composes; `server.ts` listens; tests
  use `app.inject()`. Our pino logger is passed as `loggerInstance`; Fastify's
  own per-request logging is disabled through `LogController` in favour of
  one completion line per request; request ids are generated by the platform
  (`genReqId`) and the client's `X-Request-Id` is ignored.
- Alternatives considered: (a) **Express** -- mature but slower, with an
  error-handling model built on callbacks and no first-class lifecycle hooks
  or schema support; async errors need wrappers everywhere. (b) **Hono** --
  fast and portable to edge runtimes, attractive for a future edge delivery
  function, but its Node adapter and plugin ecosystem (multipart, rate
  limiting) are younger, and edge deployment is not a v1 goal. (c) **NestJS**
  -- dependency injection and decorators add a layer the module structure in
  `docs/ENGINEERING/02` does not need.
- Consequences: Two type-level wrinkles, both commented in `app.ts`: Fastify
  5's types do not accept a numeric `trustProxy` (use addresses/CIDRs), and
  passing a pino 10 `Logger` as `loggerInstance` types the instance with that
  logger generic, so it is widened to `FastifyBaseLogger` for plugins to
  accept it. The top-level `disableRequestLogging` and `requestIdLogLabel`
  options are deprecated in 5.x (removed in 6) and are set through
  `LogController` instead. `docs/ENGINEERING/05`'s layer templates still use a
  placeholder framework; they are rewritten from the first real module
  (`P1-02`) rather than invented ahead of working code.

**Addendum to ADR-018 (2026-09-21):** bundling workspace packages while
leaving third-party packages external has a failure mode ADR-018 did not
state: the bundle imports `pg`, `pino`, ... directly, but those are
dependencies of the *packages*, not of the service, and pnpm's strict layout
(and the `pnpm deploy` output the Docker image uses) does not resolve them.
The first built api failed at startup with "Cannot find package 'pg'".
Resolution: every service declares, as its own `dependencies`, each
third-party package its bundle imports; `scripts/check-bundle-deps.mjs` scans
