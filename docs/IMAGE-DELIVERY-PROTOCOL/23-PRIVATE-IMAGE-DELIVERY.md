# 23 - Private Image Delivery

> Category: **Image Delivery Protocol** (`docs/IMAGE-DELIVERY-PROTOCOL/`) &nbsp;|&nbsp; Status: Draft specification &nbsp;|&nbsp; Owner: TBD

## Purpose

Define the full request path for a PRIVATE-visibility asset, distinct from SIGNED/EXPIRING (which are unlisted-but-time-boxed, not authenticated).
...
- PRIVATE assets require either a valid signed URL (per `21-SIGNED-URL-PROTOCOL.md`) or an authenticated session/API key with resource access -- there is no third, weaker path.
- State that PRIVATE derivatives are still cached (at origin and, where the signature scheme allows, at the edge) but the visibility check is re-run on every single request regardless of cache status -- a cache hit never bypasses authorization.
