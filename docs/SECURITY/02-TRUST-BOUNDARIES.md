...
Specify trust boundaries for the Image Management & Delivery Platform. Draw every boundary where trust level changes: public internet -> API Gateway, API Gateway -> internal services, platform -> third-party storage provider, platform -> consumer's webhook endpoint.
...
- Draw every boundary where trust level changes: public internet -> API Gateway, API Gateway -> internal services, platform -> third-party storage provider, platform -> consumer's webhook endpoint.
