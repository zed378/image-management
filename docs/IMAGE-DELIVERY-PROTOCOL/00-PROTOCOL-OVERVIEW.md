...
Introduce the Image Delivery Protocol as the platform's core contract: the concept and full lifecycle of an image request, from a consumer holding an `asset_id` to bytes leaving the CDN edge.
...
- State the protocol's one job in a sentence: turn (asset_id + parameters) into a deterministic, cacheable image response -- everything else in this category exists to make that sentence precise and testable.
- Draw the lifecycle once, end to end: consumer builds a URL -> API Gateway validates & normalizes params -> cache lookup -> (on miss) Image Processing Engine runs the pipeline -> result cached -> CDN edge serves it -> consumer receives bytes.
- State explicitly that this document is a map of the other 39, not a duplicate of their content -- it should not need updating when a single parameter's default changes.
